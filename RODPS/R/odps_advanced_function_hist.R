                        ...
                        ...)
{
 ...
 sql <- paste("select count(*), min(",colname,"), "," max(",colname,") from ",tblname,";",sep=" ")
 cat('\n')
 mm <- rodps.sql( sql )
 #nclass.Sturges
 ...
 if(right){
   cnt <- paste( "  count (case when ", colname, ">",lb," and ", colname , "<=", ub, " then 1 end)", sep="")
   cntb <- paste( "  count (case when ", colname, ">=",lb[1]," and ", colname , "<=", ub[1], " then 1 end)", sep="")
   cnt[1] <- cntb
 }
 else{
   cnt <- paste( "  count (case when ", colname, ">=",lb," and ", colname , "<", ub, " then 1 end)", sep="")
   cnte <- paste( "  count (case when ", colname, ">=",lb[length(cnt)]," and ", colname , "<=", ub[length(cnt)], " then 1 end)", sep="")
   cnt[length(cnt)] <- cnte
 }
 ...
 equi <- diff(range(diff(breaks))) < 1e-7
