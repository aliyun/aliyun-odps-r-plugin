...
#enable the MCQA function to Maxcompute
rodps.adhoc_query <- function(query) {
    .check.init();
    if (is.null(query) || query == "") {
        stop(error("input_query_error", "query is null"))
    }
    postfix <- paste(sample(c(letters[1:6], 0:9), 30, replace = TRUE), collapse = "")
    temp_filename <- paste(postfix, "_temp.csv", sep = "")
    result <- odpsOperator$runSqlWithMCQA(query, temp_filename)
    keySet <- .jrcall(result, "keySet")
    res_iter <- .jrcall(keySet, "iterator")
    aList <- list()
    key_classes <- c()
    while (.jrcall(res_iter, "hasNext")) {
        key <- .jrcall(res_iter, "next");
        aList[[key]] <- .jrcall(result, "get", key)
        key_classes = append(key_classes, key)
    }
    length = length(aList)
    classes <- c()
    for (i in 1:length) {
    if (aList[[i]] == 'DATE') {
        classes = append(classes, 'POSIXct')
    }
    else {
        classes = append(classes, NA)
        }
    }

    if (file.info(temp_filename)$size != 0) {
        data <- read.csv(file = temp_filename, stringsAsFactors = FALSE, colClasses = classes)
        ...
        data = data.frame(matrix(nrow = 0, ncol = length(key_classes)))
        colnames(data) = key_classes
    return(data)
    }
}

# enable the MCQA function to Maxcompute with out data type
# tidyverse version: dependency = tidyr, dplyr, readr
rodps.adhoc_query_readr <- function(query) {
    .check.init();
    if (is.null(query) || query == "") {
        stop(error("input_query_error", "query is null"))
    }
    postfix <- paste(sample(c(letters[1:6], 0:9), 30, replace = TRUE), collapse = "")
    temp_filename <- paste(postfix, "_temp.csv", sep = "")
    result <- odpsOperator$runSqlWithMQAC(query, temp_filename)
    keySet <- .jrcall(result, "keySet")
    res_iter <- .jrcall(keySet, "iterator")
    aList <- list()
    key_classes <- c()
    ...
    # Map list of Java classes to columns types
    cls <- unlist(aList)
    classes <- dplyr::case_when(
    cls %in% c("INT", "BIGINT") ~ "i",
    cls == "DATE" ~ "D",
    cls == "STRING" ~ "c",
    cls == "DOUBLE" ~ "d",
    cls == "BOOLEAN" ~ "l",
    ...
    # Read csv
    if (file.info(temp_filename)$size != 0) {
        data <- readr::read_csv(file = temp_filename, col_types = classes)
        unlink(temp_filename)
    if ("try-error" == class(result)) {
        stop(paste("Exception ocurred when loading table:", tmptable, sep = ""))
    }
        ...
        data <- tidyr::as_tibble(data.frame(matrix(nrow = 0, ncol = length(key_classes))))
        colnames(data) <- key_classes
        return(data)
      }
}
